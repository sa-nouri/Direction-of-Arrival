...
Ns = 1e3; % num of signals and noise
M = 10 ;
Nt = 1;
theta = [50]; 
...
R = zeros(M);
for i=1:Ns
    noise  = (randn(M,1) + ( 1i * rand(M,1))) * (sqrt(1/2));
    %     signal = (randn(N,1) + ( i * rand(N,1))) * (sqrt(SNR(1,6)/2));
    signal = rand(1,1) * (sqrt(SNR/2));
...
theta = 0:0.1:180;
f_beam = zeros(size(theta))

for i=1:length(theta)
    a = ( 1/sqrt(M) ) * exp( -1i * d' * pi .* cosd(theta(i)));
    f_beam(i) = a'*R*a;
end

plot(theta, abs(f_beam))
...
vv = exp(j * 2 * pi * d * n * u);
theta_T = 30/180 * pi;
w = 1/Ns * ones(Ns, 1) .* exp(j * n * pi * cos(theta_T));
B = w' * vv;
B = 10 * log10(abs(B).^2);
