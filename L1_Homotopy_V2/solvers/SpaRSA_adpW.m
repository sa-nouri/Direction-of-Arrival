function [x,x_debias,objective,times,debias_start,mses,taus, numA, numAt]= ...
    SpaRSA_adpW(y,A,tau,varargin)
...
%----------------------------------------------------------------
% Some modification for solving weighted L1 problem
% Added an option to perform adaptive reweighting using flag 
%
% adp_wt = adaptive reweighting 
%           1 - adaptive reweighting, 
%           0 - standard SpaRSA (default)
%
...
while keep_continuation
    ...
    resid =  A(x) - y; numA = numA+1;
    gradq = AT(resid); numAt = numAt+1;
    ...
    % [modified by Salman Asif @ Georgia Tech June 2012]
    %----------------------------------------------------------------------   
    if adp_wt
        if exist('W_new','var')
            gamma = find(abs(x)>tau);
            ewt_b = length(y)*(norm(x,2)/norm(x,1))^2;
            W_old = W_new;
            W_new = ones(length(x),1);
            if tau < 5*final_tau;
                % [alpha beta epsilon] = weight_param(5,1,x,length(y));
                % W_new = 1/alpha./(beta*(abs(x))+epsilon);
                
                W_new(gamma) = min([W_old(gamma) 1./abs(x(gamma))/ewt_b],[],2);
            else
                W_new(gamma) = min([W_old(gamma) 1./abs(x(gamma))/ewt_b],[],2);
                % W_new(gamma) = W_old(gamma)/2; % 1/2
            end
            psi_function = @(x,tau) soft(x,tau*W_new);
            phi_function = @(x) sum(abs(W_new.*x));
            % figure(115); 
            % subplot(211); plot([true x]); 
            % subplot(212); semilogy([W_new]); pause(1);
        else
            W_new = 1;
            psi_function = @(x,tau) soft(x,tau*W_new);
            phi_function = @(x) sum(abs(W_new.*x));
    ...
    while keep_going
        ...
        switch stopCriterion
            ...
            case 2,
                ...
                % function value.
                scaleFactor = norm(gradq(:)./W_new,inf);
            ...
            case 3,
                ...
                w = [ min(tau.*W_new + gradq(:), max(prev_x(:),0.0)); ...
                    min(tau.*W_new - gradq(:), max(-prev_x(:),0.0))];
...
if (debias & (sum(x(:)~=0)~=0))
    ...
    resid = A(x_debias); numA = numA+1;
    resid = resid-y;
    prev_resid = eps*ones(size(resid));
    
    rvec = AT(resid); numAt = numAt+1;
    ...
    while cont_debias_cg
        ...
        RWpvec = A(pvec);      numA = numA+1;
        Apvec = AT(RWpvec);     numAt = numAt+1;
