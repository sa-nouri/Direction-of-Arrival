...
'omega'      ,    0.3 , ... % Subroutine weight omega that modifies the LASSO subproblems %% WSPGL1 _ Hassan Mansour
'project'    , @NormL1_project , ...
'primal_norm', @NormL1_primal  , ...
'dual_norm'  , @NormL1_dual      ...
   );
options = wspgSetParms(defaultopts, options);
...
omega         = options.omega; % WSPGL1 _ Hassan Mansour
...
while 1
    ...
    % Multiple tau: Check if found root and/or if tau needs updating.
       ...
       if testUpdateTau
          ...
          if ~isempty(x)
%               omega = 0.3;
              [Cx, idx] = sort(abs(x), 'descend');
              ratCx = sqrt(cumsum(Cx.^2))/norm(Cx);
              k = min([round(0.5*length(b)/log(n/length(b))), find(ratCx >= 0.9999,1),round(length(b)/2-1)]);
%               k = round(length(b)/log(n/length(b)));
              Tsupp = idx(1:k);
              
              weights = ones(n,1);
              weights(Tsupp) = omega;
              weights = weights.*weights_old;
              
              %update tau and gnorm
              tau = options.primal_norm(x,weights);
              gNorm   = options.dual_norm(g,weights);
    ...
    %{WSPGL1 _ Hassan Mansour
    if testUpdateTau
       xNorm1_1 = [xNorm1_1; xNorm1(iter+1)];
       rNorm2_1 = [rNorm2_1; rNorm];
       lambda_1 = [lambda_1; gNorm];
    end
    %}
...
%{WSPGL1 _ Hassan Mansour
info.xNorm1_1      = xNorm1_1;
info.rNorm2_1      = rNorm2_1;
info.lambda_1      = lambda;
%}
