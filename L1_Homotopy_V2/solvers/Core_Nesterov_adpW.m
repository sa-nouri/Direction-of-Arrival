...
if strcmpi(TypeMin,'TV')
    Lmu = 8*Lmu;
    global T_frames ROW COL
    if isempty(T_frames)
        T_frames = 1;
        N = length(xplug); n = floor(sqrt(N));
        ROW = n; COL = n; 
    end
    
    %     Dv = spdiags(repmat([reshape([-ones(n-1,n); zeros(1,n)],N,1) ...
    %         reshape([zeros(1,n); ones(n-1,n)],N,1)], T_frames,1), [0 1], T_frames*N, T_frames*N);
    %     Dh = spdiags(repmat([reshape([-ones(n,n-1) zeros(n,1)],N,1) ...
    %         reshape([zeros(n,1) ones(n,n-1)],N,1)],T_frames,1), [0 n], T_frames*N, T_frames*N);
    %     D = sparse([Dh;Dv]);
    
    N = ROW*COL;
    Dv = spdiags(repmat([reshape([-ones(ROW-1,COL); zeros(1,COL)],N,1) ...
        reshape([zeros(1,COL); ones(ROW-1,COL)],N,1)], T_frames,1), [0 1], T_frames*N, T_frames*N);
    Dh = spdiags(repmat([reshape([-ones(ROW,COL-1) zeros(ROW,1)],N,1) ...
        reshape([zeros(ROW,1) ones(ROW,COL-1)],N,1)],T_frames,1), [0 ROW], T_frames*N, T_frames*N);
    D = sparse([Dh;Dv]);
...
for k = 0:maxiter-1,
   ...
   %---- Dual problem
   clear Acp AtAcp df fx uk;
   if strcmpi(TypeMin,'L1')  [df,fx,val,uk,U,Ut,normU] = Perform_L1_Constraint(xk,mu,U,Ut,length(b));end
...
function [df,fx,val,uk, U, Ut, normU] = Perform_L1_Constraint(xk,mu,U,Ut,M)
    ...
    gamma = find(xk);
    ewt_b = 2*M*(norm(xk,2)/norm(xk,1))^2;
    W_new = ones(length(xk),1);
    W_new(gamma) = min([W_new(gamma) 1./abs(xk(gamma))/ewt_b],[],2);
    
    U = U; %spdiags(W_new,0,length(xk),length(xk));
    Ut = U;
    normU = max(W_new);
end
