...
for nl=1:MaxIntIter
    ...
    [xk,niter_int,res,out,optsOut] = Core_Nesterov_adpW(...
        A,At,b,mu,delta,opts);
        
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Adaptive weights selection [sasif] %
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    gamma = find(xk);
    ewt_b = 2*length(b)*(norm(xk,2)/norm(xk,1))^2;
    W_new = ones(length(xk),1);
    W_new(gamma) = min([W_new(gamma) 1./abs(xk(gamma))/ewt_b],[],2);
    
    opts.U = spdiags(W_new,0,length(xk),length(xk));
    opts.Ut = opts.U;
...
function th=ValMUTv(x)
    ...
    if isempty(T_frames)
        T_frames = 1;
        N = length(x)/T_frames; n = floor(sqrt(N));
        ROW = n; COL = n;
    end
    N = ROW*COL;
