...
%
   if nargin > 1,
	...
	elseif strcmp(Name,'LinChirp'), 
        % SAL comment: linear chirp with frequency sweep from 0 to n/2 Hz,
        % sampled at critical frequency of n samples per second
        % Matlab chrip command is cos(2*pi*[f0 t + k/2 t^2])        
	...
	elseif strcmp(Name,'HypChirps'), % Hyperbolic Chirps of Mallat's book
		...
		enveloppe = ones(1,M); % the rising cutoff function 
            ...
            enveloppe(M-P+1:M) = fliplr(enveloppe(1:P));
  		env 	= zeros(1,n);
	...
	elseif strcmp(Name,'Chirps'), % Mixture of Chirps of Mallat's book
  	 ...
  	 enveloppe(7*n/8+1:n) = fliplr(enveloppe(1:n/8));
 		sig 	= sig.*enveloppe;
	...
	elseif strcmp(Name,'Cusp2'),
		...
		sig(M-2.5*N+2:M-1.5.*N+1) = fliplr(x);
